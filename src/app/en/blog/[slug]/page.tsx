...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical,
      languages: post.altLocaleSlug ? { 'ar': `${SITE}/blog/${post.altLocaleSlug}/` } : undefined,
    },
...
export default async function EnBlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const post = await getPost(decodeURIComponent(slug));
  if (!post) notFound();
  const related = await getRelatedPosts(post.categoryId, post.slug, 'EN', 3);

  return <BlogArticle post={post} related={related} basePath="/en/blog" altBasePath="/blog" />;
}
