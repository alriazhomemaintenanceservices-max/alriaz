...
const services: { icon: string; titleKey: TKey; descKey: TKey }[] = [
  { icon: '🔔', titleKey: 'intcom-svc-install-title', descKey: 'intcom-svc-install-desc' },
  { icon: '📹', titleKey: 'intcom-svc-camera-title', descKey: 'intcom-svc-camera-desc' },
  { icon: '🚪', titleKey: 'intcom-svc-doorbell-title', descKey: 'intcom-svc-doorbell-desc' },
  { icon: '🔐', titleKey: 'intcom-svc-access-title', descKey: 'intcom-svc-access-desc' },
  { icon: '🔧', titleKey: 'intcom-svc-repair-title', descKey: 'intcom-svc-repair-desc' },
  { icon: '⬆️', titleKey: 'intcom-svc-upgrade-title', descKey: 'intcom-svc-upgrade-desc' },
];
  ...
  return (
    <>
      ...
      <section style={{ background: 'linear-gradient(135deg, #EBF8FF 0%, #FFFFFF 100%)', padding: '60px 0' }}>
        <div className="container" style={{ maxWidth: '800px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <Button href="tel:+966508901536" variant="emergency" size="large" icon={<Phone size={20} />} onClick={() => trackPhoneClick('intercom-page')}>
              {t('area-call-now')}
            </Button>
            <Button href={`https://wa.me/966508901536?text=${encodeURIComponent(language === 'ar' ? 'مرحباً، أحتاج تركيب انتركوم في الرياض' : 'Hello, I need intercom installation in Riyadh')}`} variant="whatsapp" size="large" external icon={<WhatsAppSvg size={20} />} onClick={() => trackWhatsAppClick('intercom-page')}>
      ...
      <section className="section">
        <div className="container">
          <div className="section-header">
            ...
            <p style={{ color: 'var(--gray-500)', marginTop: '8px' }}>{t('intcom-services-subtitle')}</p>
          </div>
      ...
      <section className="section bg-light">
        <div className="container">
          <div className="section-header">
            <h2 className="section-title">{language === 'ar' ? 'انتركوم حسب المنطقة' : 'Intercom by Area'}</h2>
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '10px', maxWidth: '800px', margin: '0 auto' }}>
            {areas.map((area) => (
              <Link key={area.slug} href={`/انتركوم-${area.slug}/`}
      ...
      <section style={{ background: 'linear-gradient(135deg, var(--emergency-red) 0%, #DC2626 100%)', color: 'white', padding: '48px 0' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '1.25rem', marginBottom: '24px' }}>{t('intcom-emergency-desc')}</p>
          <Button href="tel:+966508901536" variant="secondary" size="large" icon={<Phone size={20} />} onClick={() => trackPhoneClick('intercom-emergency')} style={{ background: 'white', color: 'var(--emergency-red)' }}>
