...
import { Phone, Clock, Shield, CheckCircle, Zap, ListChecks } from 'lucide-react';
...
const factors: TKey[] = ['cost-elec-factor-1', 'cost-elec-factor-2', 'cost-elec-factor-3', 'cost-elec-factor-4'];

const serviceItems: { icon: string; titleKey: TKey; descKey: TKey }[] = [
  { icon: '🔎', titleKey: 'cost-elec-svc-1-title', descKey: 'cost-elec-svc-1-desc' },
  { icon: '🔌', titleKey: 'cost-elec-svc-2-title', descKey: 'cost-elec-svc-2-desc' },
  { icon: '💡', titleKey: 'cost-elec-svc-3-title', descKey: 'cost-elec-svc-3-desc' },
  { icon: '🔧', titleKey: 'cost-elec-svc-4-title', descKey: 'cost-elec-svc-4-desc' },
  { icon: '🏠', titleKey: 'cost-elec-svc-5-title', descKey: 'cost-elec-svc-5-desc' },
];

const faqs: { qKey: TKey; aKey: TKey }[] = [
  { qKey: 'cost-elec-faq-1-q', aKey: 'cost-elec-faq-1-a' },
  { qKey: 'cost-elec-faq-2-q', aKey: 'cost-elec-faq-2-a' },
  { qKey: 'cost-elec-faq-3-q', aKey: 'cost-elec-faq-3-a' },
];

export default function ElectricianCostPage() {
  const { t, language } = useTranslation();
  const serviceName = t('electrician');

  const whatsappUrl = `https://wa.me/966508901536?text=${encodeURIComponent(
    language === 'ar' ? 'مرحباً، أحتاج تسعيرة لخدمة كهربائي في الرياض' : 'Hello, I need a price quote for electrician service in Riyadh'
  )}`;
    ...
    "@graph": [
      {
        ...
        "url": "https://saudihomeexperts.com/services/electrician/cost-riyadh/",
        "description": t('cost-elec-subtitle'),
      },
      {
        "@type": "FAQPage",
      ...
      },
      {
        ...
        "itemListElement": [
          ...
          { "@type": "ListItem", "position": 4, "name": t('cost-elec-h1'), "item": "https://saudihomeexperts.com/services/electrician/cost-riyadh/" },
        ],
      },
    ],
  };
  ...
  return (
    <>
      <nav className="container" style={{ paddingTop: '24px', marginBottom: '30px', fontSize: '0.9rem', color: 'var(--gray-500)' }}>
        <Link href="/">{t('breadcrumb-home')}</Link> / <Link href="/services/">{t('nav-services')}</Link> / <Link href="/services/electrician/">{serviceName}</Link> / <span style={{ color: 'var(--primary-blue)', fontWeight: 600 }}>{t('cost-elec-h1')}</span>
      </nav>
      ...
      <section style={{ background: 'linear-gradient(135deg, #EBF8FF 0%, #FFFFFF 100%)', padding: '60px 0' }}>
        <div className="container" style={{ maxWidth: '800px', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '1.15rem', color: 'var(--gray-700)', lineHeight: 1.7, marginBottom: '24px' }}>
            {t('cost-elec-subtitle')}
          </p>
          <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <Button href="tel:+966508901536" variant="emergency" size="large" icon={<Phone size={20} />} onClick={() => trackPhoneClick('electrician-cost')}>
              {t('area-call-now')}
            </Button>
            <Button href={whatsappUrl} variant="whatsapp" size="large" external icon={<WhatsAppSvg size={20} />} onClick={() => trackWhatsAppClick('electrician-cost')}>
              {t('cost-elec-cta')}
            </Button>
          </div>
        </div>
      ...
      <section className="section">
        <div className="container" style={{ maxWidth: '760px' }}>
          <div className="section-header">
            <h2 className="section-title">{t('cost-elec-factors-title')}</h2>
          </div>
      ...
      <section className="section bg-light">
        <div className="container">
          <div className="section-header">
            <h2 className="section-title">{t('cost-elec-services-title')}</h2>
          </div>
      ...
      <section className="section">
        <div className="container" style={{ maxWidth: '760px' }}>
          <div style={{ padding: '24px 28px', background: '#FEF3C7', borderRadius: '12px' }}>
            <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '8px', color: 'var(--dark)' }}>{t('cost-elec-emergency-title')}</h3>
            <p style={{ color: 'var(--gray-700)', lineHeight: 1.7 }}>{t('cost-elec-emergency-desc')}</p>
          </div>
        </div>
      </section>
      ...
      <section className="section bg-light">
        <div className="container" style={{ maxWidth: '760px', textAlign: 'center' }}>
          <h2 style={{ fontSize: '1.5rem', fontWeight: 800, marginBottom: '12px', color: 'var(--dark)' }}>{t('cost-elec-quote-title')}</h2>
          <p style={{ color: 'var(--gray-600)', lineHeight: 1.7, marginBottom: '24px' }}>{t('cost-elec-quote-desc')}</p>
          <Button href={whatsappUrl} variant="whatsapp" size="large" external icon={<WhatsAppSvg size={20} />} onClick={() => trackWhatsAppClick('electrician-cost-quote')}>
            {t('cost-elec-cta')}
          </Button>
        </div>
      </section>
      ...
      <section style={{ padding: '32px 0', background: '#F0F9FF', borderTop: '2px solid #DBEAFE', borderBottom: '2px solid #DBEAFE' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', flexWrap: 'wrap' }}>
            <Link href="/services/electrician/emergency/" style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '14px 28px', background: 'white', border: '2px solid var(--gray-300)', borderRadius: '12px', fontWeight: 600, textDecoration: 'none', color: 'var(--dark)' }}>
              <Zap size={20} /> {language === 'ar' ? 'كهربائي طوارئ' : 'Emergency Electrician'}
            </Link>
            <Link href="/services/electrician/" style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '14px 28px', background: 'white', border: '2px solid var(--gray-300)', borderRadius: '12px', fontWeight: 600, textDecoration: 'none', color: 'var(--dark)' }}>
              <CheckCircle size={20} /> {language === 'ar' ? 'كل خدمات الكهرباء' : 'All Electrical Services'}
            </Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ background: 'linear-gradient(135deg, var(--primary-blue) 0%, #1D4ED8 100%)', color: 'white', padding: '40px 0' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          ...
          <Button href="tel:+966508901536" variant="secondary" size="large" icon={<Phone size={20} />} onClick={() => trackPhoneClick('electrician-cost-footer')} style={{ background: 'white', color: 'var(--primary-blue)' }}>
            {t('area-call-now')}
          </Button>
        </div>
      </section>
